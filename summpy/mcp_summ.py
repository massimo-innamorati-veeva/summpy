...
def summarize(text, char_limit, sentence_filter=None, debug=False):
    ...
    words_list = [
        ...
        w.encode("utf-8")
        for s in sents
        for w in tools.word_segmenter_ja(s)
    ]
    ...
    prob += (
        pulp.lpSum([sent_id2len[id_] * sent_vars[id_] for id_ in sent_ids])
        <= char_limit,
        "lengthRequirement",
    )
    ...
    for w in tf:
        prob += (
            pulp.lpSum([word_contain[id_][w] * sent_vars[id_] for id_ in sent_ids])
            >= word_vars[w],
            f"z:{w}",
        )
